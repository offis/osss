...
#define OSSS_GREEN // this design represents an application 
...
class Top : public osss_system
{
 public:
  ...
  Bus_ChannelType* bus_ch;
  P2P_ChannelType* bridge_to_APB_master_ch;
  
  osss_object_socket<osss_shared< Bridge, Scheduler > > *bridge;
    ...
    prdata("prdata")
  {
    ...
    bus_ch = new Bus_ChannelType("bus_ch");
    bus_ch->clock_port(clock);
    bus_ch->reset_port(reset);

    bridge_to_APB_master_ch = new P2P_ChannelType("bridge_to_APB_master_ch");
    bridge_to_APB_master_ch->clock_port(clock);
    bridge_to_APB_master_ch->reset_port(reset);

    bridge = new 
      osss_object_socket<osss_shared< Bridge, Scheduler > >( "bridge_socket" );
    bridge->clock_port( clock );
    bridge->reset_port( reset );
    bridge->bind(*bus_ch);
    bridge->bind(*bridge_to_APB_master_ch);

    master1 = new Master< 1 >( "master1" );
    master1->clock( clock );
    master1->resetn( resetn );
    master1->output( out );
    master1->bridge_write_port( *bus_ch, *bridge );
    master1->bridge_write_port.setPriority(1);
    master1->bridge_read_port( *bus_ch, *bridge );
    ...
    master2->output( out2 );
    master2->bridge_write_port( *bus_ch, *bridge );
    master2->bridge_write_port.setPriority(2);
    master2->bridge_read_port( *bus_ch, *bridge );
    ...
    apbmaster->bridge_port( *bridge_to_APB_master_ch, *bridge );
