...
OSSS_SOFTWARE_TASK(task2)
{
 ...
 public:
  ...
  void main()
  {
    ...
    // The previous RET is intentionally violated by an inner RET. 
    // Hence we expect now == 10.0 us instead of 7.0 us
    EXPECTED_TIME(sc_time(110.0, SC_NS)+ //reset time
		  sc_time(14.0, SC_US));
...
public:
  ...
  // software processors
  osss_processor* processor1;
  osss_processor* processor2;

 Top(sc_module_name name, int enableKey) : osss_system(name)
 { 
   ...
   if (enableKey & 1)
   {
     ...
     processor1 = new osss_processor("processor1");
     processor1->clock_port(pi_bClk);
     processor1->reset_port(pi_bReset);
     processor1->add_sw_task(t1);
   }
   ...
   if (enableKey & 2)
   {
     ...
     processor2 = new osss_processor("processor2");
     processor2->clock_port(pi_bClk);
     processor2->reset_port(pi_bReset);
     processor2->add_sw_task(t2);
   }
 }
