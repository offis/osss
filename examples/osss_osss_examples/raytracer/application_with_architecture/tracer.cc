...
/* The camera's left, up and look-at vectors in "world" space 
   ...
   a pin-hole camera. The film plane is locate -1.0d coordinate units in 
   g_camAtTarget direction.   
*/
...
/* Determines how far we have to go in x/y-direction on the film when moving 
   by one pixel 
*/
                 ...
                 Bitmap *dest, const Tile *tile)
{
  ...
  vec3ScaleAdd2(&g_filmBottomRight, &filmCenter, halfWidth, &camRight, 
                halfHeight, &camDown);
...
/*
  ...
  intersection for rays which directly start from an object, e.g. relfection 
  rays.
 */
static
...
{
  return   vec3sAreClose(&ir1->m_intersectionPoint, &ir2->m_intersectionPoint)
