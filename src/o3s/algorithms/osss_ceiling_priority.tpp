...
inline
osss_ceiling_priority< MaxClients >::
 ...
 schedule( const RequestVector &clientRequests )
{
  ...
  {
    if ( clientRequests[ queue[i] ] && !ripple )
    {
      grant = queue[i];
      ripple = true;
    }
    if ( ripple && ( i != numberOfClients - 1 ) )
    {
      osss_index_type swap = queue[i];
                  queue[i] = queue[i + 1];
              queue[i + 1] = swap;
    }
  }
...
// $Id: osss_ceiling_priority.tpp 2950 2009-01-05 15:59:31Z claus $
